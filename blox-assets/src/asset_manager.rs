...
use crate::{asset::Asset, error::AssetError, loader::Loader, types::AssetPath};
    ...
    pub fn load<T: Asset>(&mut self, asset_path: &AssetPath) -> anyhow::Result<T> {
        ...
        span.in_scope(|| {
            for extension in T::EXTENSIONS {
                let path = self.base_dir.join(format!("{}{}", asset_path, extension));
                ...
                if let Ok(canonical_path) = std::fs::canonicalize(&path) {
                    if canonical_path.starts_with(&self.base_dir) {
                        ...
                        error!(path = path.to_str().unwrap(), "not inside base directory");
                    }
                } else {
                    let path = path.to_string_lossy().to_string();
                    error!(path = path.as_str(), "could not canonicalize path");
                }
            }

            Err(anyhow::Error::new(AssetError::AssetNotFound(
                asset_path.clone(),
            )))
        })
    }
